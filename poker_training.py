...
def train_with_hyperparameters(network_shape, num_epochs, batch_size, learning_rate):
...
if __name__ == '__main__':
    ...
    for network_shape in network_shape_values:
        test_acc_dicts[network_shape] = {}
        for learning_rate in learning_rate_values:
            if "Batch Size" not in test_acc_dicts[network_shape]:
                test_acc_dicts[network_shape]["Batch Size"] = batch_size_values[:]
            test_acc_dicts[network_shape][learning_rate] = []
            for batch_size in batch_size_values:
                test_acc = train_with_hyperparameters(network_shape, num_epochs, batch_size, learning_rate)
                test_acc_dicts[network_shape][learning_rate].append(test_acc)
                
                print("Batch size: {batch_size}, Learning rate: {learning_rate}, Network shape: {network_shape}".format(batch_size=batch_size, learning_rate=learning_rate, network_shape=network_shape))
                if test_acc > max_test_acc:
                    max_test_acc = test_acc
                    print("New max test accuracy: ", max_test_acc)
    
    df1 = pd.DataFrame(test_acc_dicts[network_shape_values[0]])
    df2 = pd.DataFrame(test_acc_dicts[network_shape_values[1]])
    df3 = pd.DataFrame(test_acc_dicts[network_shape_values[2]])
    df4 = pd.DataFrame(test_acc_dicts[network_shape_values[3]])
